...
usage(){
    echo
    echo "Usage: $0 [-b|-r] -H <COUCHDB_HOST> -d <DB_NAME>,<DB_NAME> -f <BACKUP_FILE> [-u <username>] [-p <password>] [-P <port>] [-l <lines>] [-t <threads>] [-a <import_attempts>]"
    ...
    echo -e "\t-d   list of CouchDB Database name to backup/restore."
    ...
    echo -e "\t-v   Backup Only views from Database"
    echo -e "\t-o   output directory (Backup Only)"
    echo -e "\t-i   input directory (Restore Only)"
...
compress=false
timestamp=false
process_only_views=false
output_directory="."
input_directory="."

while getopts ":h?H:d:f:u:p:P:l:t:a:cqzTVbBrvo:i:" opt; do
    case "$opt" in
        h) usage;;
        b|B) backup=true ;;
        r) restore=true ;;
        H) url="$OPTARG" ;;
        d) list_db_name="$OPTARG" ;;
...
#split list of database
IFS=',' read -ra DATABASES <<< $list_db_name


is_first_db=true
for db_name in "${DATABASES[@]}"; do
    echo "▶️ Traitement de la base : $db_name"
    # db_name
    if [ "x$db_name" = "x" ]; then
    ...
    if [ $backup = true ]&&[ $restore = false ]; then
        ...
        if [ "$is_first_db" = true ]; then
            ...
            if [ ! -d "$output_directory" ]; then
              ...
              read -p "Do you want to clean ? (Y/N) " reponse
              case "$reponse" in
                [Yy]|[oO])
                  rm -rf "$output_directory"/*
                  echo "$output_directory" cleaned
                  ;;
                *)
                  datetime=`date "+%Y%m%d-%H%M%S"`
                  output_directory="$output_directory"/backup_"$datetime"
                  mkdir -p "$output_directory"
                  echo "INFO: new Output directory ${output_directory}"
                  ;;
              esac
            fi
            is_first_db=false
        ...
        file_name=$output_directory"/"$db_name".json"
        file_name_design=$output_directory"/"$db_name"_design.json"
        # file_name
        ...
        if [ -f ${file_name} ]; then
            ...
            read -p "Do you want to remove  it ? (Y/N) " reponse
            case "$reponse" in
                [Yy]|[oO])
                  rm -f ${file_name}
                  ...
                  ;;
                *)
                  echo ${file_name} has not been modified.
                  continue
                  ;;
              esac
            
        fi 
        #echo ${curlSilentOpt} ${curlopt} -X GET "$url/$db_name/_all_docs?include_docs=true&attachments=true"  
        getOnlyViews="&startkey=\"_design/\"&endkey=\"_design0\""
        
        # Grab our data from couchdb
        if [ "$process_only_views" = false ]; then
           curl ${curlSilentOpt} ${curlopt} -X GET "$url/$db_name/_all_docs?include_docs=true&attachments=true" -o ${file_name}
        else
            #echo "backup only views"
            curl ${curlSilentOpt} ${curlopt} -X GET "$url/$db_name/_all_docs?include_docs=true&attachments=true$getOnlyViews" -o ${file_name}
        fi
        ...
        if [ "$process_only_views" = false ]; then
           $echoVerbose && echo "... INFO: Export completed successfully. File available at: ${file_name}"
            ...
            $echoVerbose && echo "... INFO: Export only views completed successfully. File available at: ${file_name}"
        fi
    ...
    ### Else if user selected Restore:
        ...
        file_name=$input_directory"/"$db_name".json"
        if [ ! -f ${file_name} ]; then
