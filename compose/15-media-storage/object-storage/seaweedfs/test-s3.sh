#!/usr/bin/env bash
# POC S3 SeaweedFS — smoke test de l'API S3 via AWS CLI.
# Prérequis : awscli installé. Aucune config globale requise (creds inline).
#   ./test-s3.sh
set -euo pipefail
...
echo "2) cp (put object)";         printf 'hello seaweedfs %s\n' "$(date)" > /tmp/poc-seaweed.txt; s3 s3 cp /tmp/poc-seaweed.txt "s3://$BUCKET/poc.txt"
echo "3) ls (list)";               s3 s3 ls "s3://$BUCKET/"
echo "4) cp (get object)";         s3 s3 cp "s3://$BUCKET/poc.txt" /tmp/poc-seaweed-get.txt && echo "   contenu: $(cat /tmp/poc-seaweed-get.txt)"
